 ...
 */ class Game {
   ...
   */ constructor(initialState){
        if (initialState) this.initialState = initialState.map((row)=>[
                ...row
            ]);
   ...
   */ restart() {
    ...
    move(direction) {
        ...
        if (direction === "left" || direction === "right") for(let rowIndex = 0; rowIndex < 4; rowIndex++){
            let line = [
                ...this.state[rowIndex]
            ];
        ...
        else if (direction === "up" || direction === "down") for(let colIndex = 0; colIndex < 4; colIndex++){
            let line = [];
            for(let rowIndex = 0; rowIndex < 4; rowIndex++)line.push(this.state[rowIndex][colIndex]);
            if (direction === "down") line.reverse();
            line = this.mergeLine(line);
            if (direction === "down") line.reverse();
            for(let rowIndex = 0; rowIndex < 4; rowIndex++)this.state[rowIndex][colIndex] = line[rowIndex];
        }
    ...
    mergeLine(line) {
        ...
        const result = [];
        let index = 0;
        while(index < numbers.length)if (index + 1 < numbers.length && numbers[index] === numbers[index + 1]) {
    ...
    addStartingTile() {
        ...
        for(let rowIndex = 0; rowIndex < 4; rowIndex++){
            for(let colIndex = 0; colIndex < 4; colIndex++)if (this.state[rowIndex][colIndex] === 0) emptyCells.push([
                rowIndex,
                colIndex
            ]);
        }
